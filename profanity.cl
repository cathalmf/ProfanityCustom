...
__kernel void profanity_score_leading(__global mp_number * const pInverse, __global result * const pResult, __constant const uchar * const data1, __constant const uchar * const data2, const uchar scoreMax) {
	...
	int hashMax = 0;
	int sequenceScore = 0;
	int lettersOnlyScore = 0;
	const uchar minChar = 10;
	const uchar maxChar = 15;

	for(int i = 0; i< 20; ++i) {

		// checks for occurances of dead
		
		if(i < 19 && (hash[i] & 0xF0) >> 4 == data1[13]
			&& (hash[i] & 0x0F) == data1[14]
			&& (hash[i+1] & 0xF0) >> 4 == data1[10]
			&& (hash[i+1] & 0x0F) == data1[13])
		{
			sequenceScore += 4;
		}
		...
		if(i < 19 && (hash[i] & 0xF0) >> 4 == data1[11]
			&& (hash[i] & 0x0F) == data1[10]
			&& (hash[i+1] & 0xF0) >> 4 == data1[11]
			&& (hash[i+1] & 0x0F) == data1[14])
		{
			sequenceScore += 4;
		}

		// occurances of abcdef

		if(i < 18 && (hash[i] & 0xF0) >> 4 == data1[10]
			&& (hash[i] & 0x0F) == data1[11]
			&& (hash[i+1] & 0xF0) >> 4 == data1[12]
			&& (hash[i+1] & 0x0F) == data1[13]
			&& (hash[i+2] & 0x0F) >> 4 == data1[14]
			&& (hash[i+2] & 0x0F) == data1[15])
		{
			sequenceScore += 6;
		}
		
		// checks for sequential 0-9

		if(i < 16 && (hash[i] & 0xF0) >> 4 == data1[0]
			&& (hash[i] & 0x0F) == data1[1]
			&& (hash[i+1] & 0xF0) >> 4 == data1[2]
			&& (hash[i+1] & 0x0F) == data1[3]
			&& (hash[i+2] & 0xF0) >> 4 == data1[4]
			&& (hash[i+2] & 0x0F) == data1[5]
			&& (hash[i+3] & 0xF0) >> 4 == data1[6]
			&& (hash[i+3] & 0x0F) == data1[7]
			&& (hash[i+4] & 0xF0) >> 4 == data1[8]
			&& (hash[i+4] & 0x0F) == data1[9])
		{
			sequenceScore += 10;
		}

		// check for sequence 9-0

		if(i < 16 && (hash[i] & 0xF0) >> 4 == data1[9]
			&& (hash[i] & 0x0F) == data1[8]
			&& (hash[i+1] & 0xF0) >> 4 == data1[7]
			&& (hash[i+1] & 0x0F) == data1[6]
			&& (hash[i+2] & 0xF0) >> 4 == data1[5]
			&& (hash[i+2] & 0x0F) == data1[4]
			&& (hash[i+3] & 0xF0) >> 4 == data1[3]
			&& (hash[i+3] & 0x0F) == data1[2]
			&& (hash[i+4] & 0xF0) >> 4 == data1[1]
			&& (hash[i+4] & 0x0F) == data1[0])
		{
			sequenceScore += 10;
		}

		// check for sequence abcdef0123456789

		if(i < 13 && (hash[i] & 0xF0) >> 4 == data1[10]
			&& (hash[i] & 0x0F) == data1[11]
			&& (hash[i+1] & 0xF0) >> 4 == data1[12]
			&& (hash[i+1] & 0x0F) == data1[13]
			&& (hash[i+2] & 0x0F) >> 4 == data1[14]
			&& (hash[i+2] & 0x0F) == data1[15]
			&& (hash[i+3] & 0xF0) >> 4 == data1[0]
			&& (hash[i+3] & 0x0F) == data1[1]
			&& (hash[i+4] & 0xF0) >> 4 == data1[2]
			&& (hash[i+4] & 0x0F) == data1[3]
			&& (hash[i+5] & 0xF0) >> 4 == data1[4]
			&& (hash[i+5] & 0x0F) == data1[5]
			&& (hash[i+6] & 0xF0) >> 4 == data1[6]
			&& (hash[i+6] & 0x0F) == data1[7]
			&& (hash[i+7] & 0xF0) >> 4 == data1[8]
			&& (hash[i+7] & 0x0F) == data1[9])
		{
			sequenceScore += 16;
		}
		...
		// score on letters only

		if (first >= minChar && first <= maxChar) {
			++lettersOnlyScore;
		}

		if (second >= minChar && second <= maxChar) {
			++lettersOnlyScore;
		}
	}
	...
	if(internalMax >= 10){
		internalMax = sequenceScore + internalMax;
	}
	else {
		internalMax = max(internalMax, sequenceScore);
	}

	
	//internalMax = max(internalMax, sequenceScore);
	//internalMax = max(internalMax, babeScore);
	//internalMax = max(internalMax, lettersScore);

	if(lettersOnlyScore == 40) {
		internalMax = lettersOnlyScore;
	}

	//if(internalMax >= 10) {
		profanity_result_update(id, hash, pResult, internalMax, scoreMax);
	//}
}
