...
std::map<std::string, bool> printedResultsMapping;
...
std::string getEthBalance(const std::string& address, const std::string& rpcEndpoint) {
    CURL* curl;
    CURLcode res;
	...
	std::string addr2 = "0x" + address;

    if (curl) {

        std::string jsonRpcRequest = R"(
            {
                "jsonrpc": "2.0",
                "method": "eth_getBalance",
                "params": [")" + addr2 + R"(", "latest"],
                "id": 1
            }
        )";

		//std::cout << "Request: " << jsonRpcRequest << std::endl;

        std::string url = rpcEndpoint;

        curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, jsonRpcRequest.c_str());
        ...
        // Note: You may want to use a JSON parsing library for a robust implementation

        // For simplicity, let's assume the balance is directly in the "result" field
        size_t resultStart = response.find("\"result\":\"") + 10;
        size_t resultEnd = response.find("\"", resultStart);
        std::string balanceStr = response.substr(resultStart, resultEnd - resultStart);

        curl_easy_cleanup(curl);
        curl_global_cleanup();

		return balanceStr;
        // Convert the hexadecimal balance to double
        //return std::stoull(balanceStr, 0, 16) / 1e18;
    }
...
static void printResult(cl_ulong4 seed, cl_ulong round, result r, cl_uchar score, const std::chrono::time_point<std::chrono::steady_clock> & timeStart, const Mode & mode) {
	...
	if(printedResultsMapping[strPublic]) {
		return;
	}

	printedResultsMapping[strPublic] = true;
...
void Dispatcher::handleResult(Device & d) {
	for (auto i = PROFANITY_MAX_SCORE; i >= 10; --i) {
